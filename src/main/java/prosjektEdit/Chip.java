...
public class Chip implements ChipInterface{
    //type (saks pose stein)
    private Character rpc='n';
    private int damage;

    //random til bruk for rolling av type og dmg
    private Random r= new Random();
    private List<Character> temp =List.of('N','R','P','C');

    //dispersion checker og innenfor hvilken range
    //random typing og mulig healing
    ...
    public Chip(){
        //roller for dispersion
        int dispersionRoll=r.nextInt(5);

        //velger randdouble mellom 50-80
    ...
    public void inflict(P person) {
        //trigger chip til å inflicte target

        //multiplier check for rpc
        double multi= checkMultiplier(person);
        
        //printer notice ved aktiv multiplier
        if (!(multi==1)) {
        ...
        System.out.println();
        //trigger person til å motta chip dmg
        person.recieve(out);

    }

    public void inflictMulti(P person, int multi) {
        //multiplier vanlig output med int multi (unused)
        //skulle brukes til en wave boss
